...
class Flowers(ImageFolder):  # inherits `ImageFolder` from `torchvision.datasetsA`
    def __init__(self, root, train=True, transform=None, **kwargs):
        ...
        self.transform = transform                         # image transformation pipeline (e.g. resize, crop, rotation)
        label_path = os.path.join(root, 'imagelabels.mat')  # Matlab files containing labels for all images
        split_path = os.path.join(root, 'setid.mat')       # Matlab files containing indices of images split into train/val/test sets
...
def build_dataset(is_train, args, folder_name=None):
    ...
    if args.data_set == 'CIFAR10':
        ...
        split = 'train' if is_train else folder_name  # folder_name can be 'gallery' or 'query'
        transform = build_reid_transform(is_train=(split == 'train'), input_size=args.input_size)
        dataset = ATRWReID(args.data_path, split=split, transform=transform)
        nb_classes = dataset.nb_classes
